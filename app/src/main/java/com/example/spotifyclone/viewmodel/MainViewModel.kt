...
@HiltViewModel
class MainViewModel @Inject constructor(private val authRepository: AuthRepository, private val prefs: Prefs) :
    ViewModel() {
                    ...
                    is BaseResponse.Success -> {
                        response.data?.also { authResponse ->
                            ...
                            authResponse.expireTime?.also { time ->
                                prefs.putInt(PrefsKeys.EXPIRES_AT , Calendar.getInstance().get(Calendar.SECOND) + time)
                            }
